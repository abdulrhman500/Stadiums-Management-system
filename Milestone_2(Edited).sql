...
GO
------------------------- <<<<<<<<<<<<<<<<<<<<<<< oupts duplicates 
select * from availableMatchesToAttend ('2012-09-1 20:00');
CREATE FUNCTION availableMatchesToAttend 
(@time DATETIME)
RETURNS TABLE
AS
    RETURN (SELECT c1.name as host, c2.name as guest , m.starting_time as startTime, s.name as stadium
			FROM Match m 
...
-- SEQUENCES
--exec purchaseTicket '22'  ,'Al-Ahly' ,'Zamalek','2019-08-1 20:00';
go
...
RETURN (
SELECT hc.name as host_Club , gc.name as guest_club ,sum(T.id)
FROM Ticket_purchase TP 
...
where M.starting_time < CURRENT_TIMESTAMP --- of all played matches  << --<needs a check>
GROUP BY hc.name, gc.name, M.id
ORDER BY sum(T.id) DESC
);
