...
CREATE VIEW matchesPerTeam AS
    SELECT COUNT(*) AS matchesPerTeam
    FROM Match m
	JOIN Club c ON c.id = m.host_club OR c.id = m.guest_club
	GROUP BY c.name, m.starting_time
	HAVING m.starting_time < CURRENT_TIMESTAMP;
GO
...
RETURN (SELECT @club as Host_club ,C.name as Guest_Club 
FROM Request RE 
JOIN Match M on M.id = RE.Match_id AND M.host_club = (SELECT c.id FROM club c WHERE c.name=@club)
JOIN Stadium S on S.id = M.stadium_id AND s. name = @stadium
JOIN Club C on M.guest_club =c.id 
	);
GO
